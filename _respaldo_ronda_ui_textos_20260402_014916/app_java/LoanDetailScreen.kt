...
fun LoanDetailScreen(
    ...
    if (currentLoan == null) {
        Column(
            ...
            AppTopBack(title = "Detalle del prÃƒÂ©stamo", onBack = { navController.popBackStack() })
            AppSectionCard { Text("No hay un prÃƒÂ©stamo seleccionado.") }
    ...
    AppConfirmDialog(
        ...
        title = "Eliminar prÃƒÂ©stamo",
        message = "Ã‚Â¿Seguro que deseas enviar a papelera el prÃƒÂ©stamo de ${currentLoan.fullName.ifBlank { "este cliente" }}? PodrÃƒÂ¡s restaurarlo desde la papelera durante 30 dÃƒÂ­as.",
    ...
    AppConfirmDialog(
        ...
        message = "Se marcarÃƒÂ¡ el prÃƒÂ©stamo como cobrado y el pendiente quedarÃƒÂ¡ en cero.",
        confirmText = "Confirmar",
        dismissText = "Cancelar",
        onConfirm = {
            val ok = sessionStore.markLoanAsCollected(currentLoan.id)
            feedback = if (ok) "PrÃƒÂ©stamo marcado como cobrado." else "No se pudo marcar como cobrado."
    ...
    AppConfirmDialog(
        ...
        message = "El prÃƒÂ©stamo quedarÃƒÂ¡ en estado perdido.",
        confirmText = "Confirmar",
        dismissText = "Cancelar",
        onConfirm = {
            val ok = sessionStore.markLoanAsLost(currentLoan.id)
            feedback = if (ok) "PrÃƒÂ©stamo marcado como perdido." else "No se pudo marcar como perdido."
            confirmLost = false
            reload()
        },
    ...
    Column(
        ...
        AppTopBack(
            title = currentLoan.fullName.ifBlank { "Detalle del prÃƒÂ©stamo" },
        ...
        AppSectionCard {
            Text("Identidad del prÃƒÂ©stamo", style = MaterialTheme.typography.titleMedium)
            ...
            BoxWithConstraints(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    DetailInfoCardRefined("CÃƒÂ©dula", currentLoan.idNumber.ifBlank { "-" }, Modifier.width(cardWidth))
                    DetailInfoCardRefined("TelÃƒÂ©fono", currentLoan.phone.ifBlank { "-" }, Modifier.width(cardWidth))
                }
            }
        }

        AppSectionCard {
            Text("Estado del prÃƒÂ©stamo", style = MaterialTheme.typography.titleMedium)
            ...
            BoxWithConstraints(
                ...
                Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        DetailInfoCardRefined("PrÃƒÂ©stamo", currentLoan.loanDate.ifBlank { "-" }, Modifier.width(cardWidth))
                ...
                onClick = {
                    ...
                    when {
                        amount == null || amount <= 0.0 -> {
                            feedback = "Debes colocar un monto vÃƒÂ¡lido."
                        }
        ...
        AppSectionCard {
            ...
            if (paymentHistory.isEmpty()) {
                AppMutedText("TodavÃƒÂ­a no hay pagos registrados para este prÃƒÂ©stamo.")
            } else {
                paymentHistory.forEachIndexed { index, record ->
                    ...
                    Text(
                        text = "${record.paymentDate} Ã‚Â· ${record.paymentType}",
        ...
        AppSectionCard {
            Text("Acciones del prÃƒÂ©stamo", style = MaterialTheme.typography.titleMedium)
            ...
            BoxWithConstraints(
                ...
                Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        DetailActionCardRefined("Preparar cobro", Modifier.width(cardWidth)) {
                            navController.navigate(AppRoutes.LoanCollectionNotice)
                        }
                        DetailActionCardRefined("Editar prÃƒÂ©stamo", Modifier.width(cardWidth)) {
        ...
        AppSectionCard {
            ...
            AppMutedText("Eliminar este prÃƒÂ©stamo borrarÃƒÂ¡ tambiÃƒÂ©n su historial de pagos guardado.")
            AppDangerButton(
                text = "Eliminar prÃƒÂ©stamo",
                onClick = {
                    feedback = ""
                    confirmDelete = true
                }

...
fun validateIdNumberOptional(value: String, fieldLabel: String = "La cÃ©dula"): String? {
    if (value.isBlank()) return null
    val digits = value.filter(Char::isDigit)
    return when {
        digits.length !in 5..15 -> "$fieldLabel debe tener entre 5 y 15 dÃ­gitos."
        else -> null
    }
}

fun validatePhoneOptional(value: String, fieldLabel: String = "El telÃ©fono"): String? {
    if (value.isBlank()) return null
    val digits = value.filter(Char::isDigit)
    return when {
        digits.length !in 7..15 -> "$fieldLabel debe tener entre 7 y 15 dÃ­gitos."
        else -> null
    }
}
...
fun validateTextMaxLength(value: String, maxLength: Int, fieldLabel: String): String? {
    return if (value.trim().length > maxLength) {
        "$fieldLabel supera el mÃ¡ximo permitido de $maxLength caracteres."
    } else {
        null
    }
}
...
fun validateLoanForm(
    ...
    if (!amount.isFinite()) return "El monto prestado no es vÃ¡lido."
    if (amount > 1_000_000_000.0) return "El monto prestado es demasiado alto."

    if (percentage == null || percentage < 0.0) return "El porcentaje no es vÃ¡lido."
    if (!percentage.isFinite()) return "El porcentaje no es vÃ¡lido."
    if (percentage > 1000.0) return "El porcentaje es demasiado alto."

    val loanLocalDate = parseIsoDateOrNull(loanDate) ?: return "La fecha del prÃ©stamo no es vÃ¡lida."
    val dueLocalDate = parseIsoDateOrNull(dueDate) ?: return "La fecha de vencimiento no es vÃ¡lida."

    if (dueLocalDate.isBefore(loanLocalDate)) {
        return "La fecha de vencimiento no puede ser menor que la fecha del prÃ©stamo."
    }

    if (exchangeRate.isNotBlank()) {
        val exchangeValue = parseMoneyOrNull(exchangeRate)
        if (exchangeValue == null || exchangeValue <= 0.0) {
            return "La tasa de cambio no es vÃ¡lida."
        }
    }
    ...
    if (existingPaidAmount > newTotal) {
        return "No puedes guardar un prÃ©stamo cuyo nuevo total quede por debajo de lo ya abonado."
    }
...
fun validateProfileForm(
    ...
    validateIdNumberOptional(idNumber)?.let { return it }
    validatePhoneOptional(phone, "El telÃ©fono principal")?.let { return it }
    validatePhoneOptional(communicationPhone, "El telÃ©fono de comunicaciÃ³n")?.let { return it }
    validatePhoneOptional(mobilePaymentPhone, "El telÃ©fono de pago mÃ³vil")?.let { return it }
...
fun validateFrequentUserForm(
    ...
    validateIdNumberOptional(idNumber)?.let { return it }
    validatePhoneOptional(phone)?.let { return it }
    validatePhoneOptional(mobilePaymentPhone, "El pago mÃ³vil")?.let { return it }
    validateTextMaxLength(bankName, MAX_BANK_NAME_LENGTH, "El banco")?.let { return it }
    validateBankAccountOptional(bankAccount)?.let { return it }
    ...
    if (bankAccount.isNotBlank() && bankName.isBlank()) {
        return "Si colocas una cuenta bancaria, indica tambiÃ©n el banco."
    }
...
fun validateReferralForm(
    ...
    if (parseIsoDateOrNull(referralDate) == null) return "La fecha del referido no es vÃ¡lida."
    ...
    if (referredBy.isBlank()) return "Debes indicar quiÃ©n lo refiriÃ³."
    ...
    if (loanAmountText.isNotBlank()) {
        if (!amount.isFinite() || amount <= 0.0) {
            return "El monto del prÃ©stamo no es vÃ¡lido."
        }
    }

    if (commissionPercent == null || commissionPercent < 0.0 || !commissionPercent.isFinite()) {
        return "El porcentaje de comisiÃ³n no es vÃ¡lido."
    }

    if (commissionPercent > 100.0) {
        return "El porcentaje de comisiÃ³n no puede ser mayor que 100."
    }

    if (status !in setOf("PENDIENTE", "PAGADA", "ANULADA")) {
        return "El estado del referido no es vÃ¡lido."
    }

...
private data class LoanSummaryItem(
    val title: String,
    val value: String,
    val subtitle: String
)
...
fun LoansScreen(
    ...
    val totalOverdue = orderedLoans.count { isLoanOverdueRefined(it) }

    val summaryItems = listOf(
        LoanSummaryItem("Cantidad", orderedLoans.size.toString(), "Resultados"),
        LoanSummaryItem("Prestado", formatMoney(totalLoaned), "Capital"),
        LoanSummaryItem("A cobrar", formatMoney(totalToCollect), "Total"),
        LoanSummaryItem("Pagado", formatMoney(totalPaid), "Abonado"),
        LoanSummaryItem("Pendiente", formatMoney(totalPending), "Saldo"),
        LoanSummaryItem("Vencidos", totalOverdue.toString(), "Con atraso")
    )

    AppConfirmDialog(
        visible = pendingDeleteLoan != null,
        title = "Eliminar prÃƒÆ’Ã‚Â©stamo",
...
@Composable
private fun LoanSummaryCardRefined(
    ...
    AppSectionCard(
        ...
        BoxWithConstraints(
            ...
            Column(
                ...
                LoanActionBoxRefined("Eliminar", Modifier.fillMaxWidth()) { onDelete() }
            }
        }
    }
}
...
private fun buildLoanDeleteMessageRefined(loan: ManualLoanData?): String {
    ...
    val name = loan.fullName.ifBlank { "este prÃƒÆ’Ã‚Â©stamo" }
    return "¿Seguro que deseas enviar a papelera el préstamo de $name? Podrás restaurarlo desde la papelera durante 30 días."
}
